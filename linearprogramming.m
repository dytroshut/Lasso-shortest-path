...
%% Linear programming
Aeq = [Incidence -Incidence];
A = sparse(Aeq);
Costw = [weights_opt; weights_opt];

Id = eye(size(A,2),size(A,2));
Ze = zeros(size(A,2),1); 

opts = optimoptions(@linprog,'Display','off','Algorithm','interior-point');
tic
x = linprog(Costw',-Id,Ze,A,Y);
toc


%%
% cost = rand(size(FullI,2),1);
% bicost = [cost;cost];
% 
% y = zeros(n,1);
% 
...
% y(14) = -1;
% 
% Id = eye(size(biI,2),size(biI,2));
% Ze = zeros(size(biI,2),1); 
% 
% x = linprog(bicost,-Id,Ze,biI,y)
